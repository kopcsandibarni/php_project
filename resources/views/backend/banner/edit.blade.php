...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('banner.update',$banner->id)}}">
        ...
        <input id="inputTitle" type="text" name="title" placeholder="Adj meg egy címet"  value="{{$banner->title}}" class="form-control">
        @error('title')
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($banner->status=='active') ? 'selected' : '')}}>Aktív</option>
            <option value="inactive" {{(($banner->status=='inactive') ? 'selected' : '')}}>Inaktív</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
        ...
        <div class="form-group mb-3">
           <button class="btn btn-success" type="submit">Frissítés</button>
        </div>
      </form>
    </div>
</div>
